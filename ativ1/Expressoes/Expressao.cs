...
public class Complexos : Expressao
{
    Expressao real;
    Expressao i;
    public Complexos(Expressao r, Expressao i){
        this.real = r;
        this.i = i;
    }
    public override string ToString() => $"({real.ToString()} + {i.ToString()}i)";
    public override Expressao Simplificar()
    {
        if(i is Numero){
            if ((this.i as Numero).valor == 0)
            {
                return real;
            }
        }
    ...
    public override Expressao Derivar(Simbolo x){
        return new Soma(real.Derivar(x), i.Derivar(x));
    }
}
